...
export default function JobItem({ job, onView, onEdit, onDelete }) {
  ...
  const { externalJobId, title, company,source, postedDate, experience,jobType, salary,skills } =
    job;
      ...
      <Card>
        ...
        <Stack sx={{ p: 3, pb: 2 }}>
          ...
          <ListItemText
            sx={{ mb: 1 }}
            primary={
              <Link component={RouterLink} href={paths.dashboard.job.details(externalJobId)} color="inherit">
                {title}
              </Link>
            }
