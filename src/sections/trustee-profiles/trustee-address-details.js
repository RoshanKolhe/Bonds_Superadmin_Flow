...
import { useGetAddressDetails } from 'src/api/trusteeKyc';

export default function TrusteeAddressNewForm({ onClose, data }) {
    const { enqueueSnackbar } = useSnackbar();
    const [isUploading, setIsUploading] = useState(false);
    const trusteeId = data?.id
    const { registeredAddress, correspondenceAddress, addressDetailsLoading } = useGetAddressDetails(trusteeId);
    ...
    const handleStatusUpdate = async (type, reason = null) => {
        try {
            ...
            await axiosInstance.patch(`/trustee-profiles/${trusteeId}/address-details-verification`, payload);
...
TrusteeAddressNewForm.propTypes = {
